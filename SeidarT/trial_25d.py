...
class Domain:
    ...
    def para_check(self):
        ...
        if self.exit_status == 0:
            self.dim = float(domain.dim[0])
            self.nx = int(domain.nx[0])
            self.nz = int(domain.nz[0])
    ...
    def para_check(self):
        ...
        if check == 0:
            ...
            for ind in range(0, self.material_list.shape[0]):
                if self.material_list[ind, 7] == 'True' and not self.material_list[ind, 8] or self.material_list[
                    ind, 8] == 'n/a':
...
if seismic.exit_status == 0 and not seismic.compute_coefficients:
    ...
    if model_type == 's' or model_type == 'b':
        ...
        if domain.dim == 2.5:
            ...
            src = np.array([seismic.x / domain.dx, seismic.y / domain.dy, seismic.z / domain.dz]).astype(int)
            ...
            seis25d.seismicfdtd25d.stiffness_write(domain.geometry + 1, seismic.tensor_coefficients,
